...
import { WebSocketLogging } from "src/utils/middlewares/web-socket-logger.middleware";

@WebSocketGateway({ cors: true })
@UseInterceptors(WebSocketLogging)
export class ChatRoomsGateway implements OnGatewayInit {
  ...
  async handleJoinChatRoom(
    ...
    const isAlreadyJoined =
      await this.chatRoomsService.isUserParticipatedInChatRoom({
        chatRoomId,
        userId,
      });
    ...
    if (isAlreadyJoined) {
      this.logger.warn(
        `User ${userName} already joined chat room ${chatRoomId}`,
      );
      return;
    }

    await this.chatRoomsService.addParticipantToChatRoom({
      chatRoomId,
      userId,
    });
  ...
  async handleNewMessage(
    ...
    if (!isParticipant) {
      throw new ForbiddenException(
        `User ${userName} not participant chat room ${chatRoomId}`,
      );
    }

    const chat = await this.chatRoomsService.addChatToChatRoom({
      chatRoomId,
      userId,
      message,
    });
  ...
  async handleDeleteMessage(
    ...
    const isChatBelongsToUser = await this.chatRoomsService.isChatBelongsToUser(
      { chatId, userId },
    );
